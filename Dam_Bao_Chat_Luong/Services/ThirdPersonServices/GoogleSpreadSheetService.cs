...
public class GoogleSpreadSheetService
{
    ...
    // --- THÊM VÀO CUỐI CLASS GoogleSpreadSheetService ---

    // Hàm này giúp tải nội dung CSV thô từ GID bất kỳ
    public async Task<string> GetCsvContentAsync(string gid)
    {
        var url = $"https://docs.google.com/spreadsheets/d/{_spreadsheetId}/export?format=csv&gid={gid}";
        return await _httpClient.GetStringAsync(url);
    }

    // Hàm này giúp ghi dữ liệu vào 1 ô bất kỳ (Dùng chung cho mọi Sheet)
    public async Task UpdateCellAsync(string sheetName, string column, int row, string value)
    {
        var clientId = _configuration?["Google:ClientId"];
        var clientSecret = _configuration?["Google:ClientSecret"];
    ...
    // Hàm này lấy quyền xác thực dùng chung cho cả Sheets và Drive
