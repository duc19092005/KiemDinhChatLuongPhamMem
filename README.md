...
Dự án hỗ trợ 2 cách chạy tùy thuộc vào mục đích sử dụng của bạn:

### Cách 1: Chạy Test Tự Động (Khuyên dùng) - Dùng MSTest Framework
Đây là cách tiêu chuẩn để chạy tự động kiểm thử tính năng đăng nhập, lấy data từ Google Sheets và trả báo cáo về Sheets.

Mở terminal/powershell tại thư mục gốc của project (nơi chứa file đuôi `.sln`) và chạy:

```bash
# Build toàn bộ solution trước
dotnet build

# Chạy TẤT CẢ các test case hiện có:
dotnet test Dam_Bao_Chat_Luong.Tests

# --- HOẶC CHẠY THEO NHÓM (LỌC) ---
# Chạy nhóm tính năng Login chung:
dotnet test Dam_Bao_Chat_Luong.Tests --filter "TestCategory=Login"

# Chỉ chạy bài test cho tài khoản Admin:
dotnet test Dam_Bao_Chat_Luong.Tests --filter "TestCategory=Admin"

# Chỉ chạy các bài test tiêu cực (nhập sai pass, để trống...):
dotnet test Dam_Bao_Chat_Luong.Tests --filter "TestCategory=Negative"
```

### Cách 2: Khởi tạo WebDriver (Chạy qua Console App)
